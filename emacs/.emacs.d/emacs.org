...
** Mini Buffer 补全(vertico-consult)
#+begin_src emacs-lisp
;; Enable Vertico.
(use-package vertico
  :custom
...
+为了启动速度，exec-path-from-shell参考文档以非交互方式启动shell，如果无法获取到换进变量，确认是否配置在zshrc或者bashrc，迁移到 ~~/.zshenv~ 或 ~~/.profile~ 即可。 [[https://github.com/purcell/exec-path-from-shell][doc]]+
golang的环境变量使用 ~go env GOPATH~ ，但是在 ~.zshenv~ 中找不到go，待解决
...
#+begin_src emacs-lisp :tangle nil
(defun doulii/my-treemacs-in-this-buffer-watcher ()
...
(let ((f (doulii/my-treemacs-in-this-buffer-watcher)))
...
(remove-variable-watcher
 'treemacs--in-this-buffer
 #'doulii/my-treemacs-in-this-buffer-watcher)
#+end_src
...
;; (use-package envrc
;;   :hook (python-mode . envrc-global-mode))

;; 需要先安装direnv https://direnv.net
;; (use-package envrc
...
使用方式，一下使用默认 ~C-c c~ 作为快捷键前缀
- ~C-c c c~ 启动claude
  - ~C-c c d~ 启动claude，指定目录，默认是使用project.el的目录
- ~C-c c k~ 退出claude
- ~C-c c s~ 使用minibuffer发送prompt
- ~C-c c x~ 发送prompt，包含文件和行数
- ~C-c c r~ 发送选中区域，如果无选中，则发送整个buffer
  - ~C-u C-c c r~ 额外指定context
- ~C-c c o~ 发送当前buffer对应的文件
- ~C-c c e~ 修复flymake/flycheck的错误 ~claude-code-fix-error-at-point~
- ~C-c c t~ 显示隐藏claude buffer ~claude-code-toggle~
- ~C-c c b~ 调整到claude buffer，隐藏的的会显示
- ~C-c c z~ readonly模式，用于复制文本
- 响应claude的提示
  - ~C-c c y~ 回车
  - ~C-c c n~ 回复no
  - ~C-c c 1~ 选型1，其他2/3同理
